...
export default function RelatedLinks({ current }: { current?: CompanyId }) {
  ...
  return (
    <section aria-labelledby="related-heading" className="mt-20 lg:mt-28">
      <h2 id="related-heading" data-reveal-lines className="display text-[clamp(1.75rem,3vw,2.375rem)]">
        ...
        {others.map((company) => (
          <li key={company.id}>
            <Link
              href={company.path}
              data-spot
              className="group relative flex h-full flex-col justify-between rounded-2xl border border-line p-6 no-underline transition-colors hover:border-[rgb(255_255_255/0.28)]"
            >
